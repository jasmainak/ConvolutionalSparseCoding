...
def learn_conv_sparse_coder(b, size_kernel, lambda_residual, lambda_prior, max_it, tol):
    ...
    # [gamma_heuristic / 2000, gamma_heuristic];
    gammas_D = [gamma_heuristic / 5000, gamma_heuristic]
    # [gamma_heuristic / 2, gamma_heuristic];
    ...
    for i in range(max_it):
        ...
        for i_d in range(max_it_d):
            ...
            v_D[0] = np.real(
                ifft2(np.sum(d_hat_dot_z_hat, axis=1).reshape(size_x)))
            v_D[1] = d
            ...
            # d = ( sum_j(gamma_j * H_j'* H_j) )^(-1) * ( sum_j(gamma_j * H_j'*
            # xi_j) )
        ...
        for i_z in range(max_it_z):
            ...
            # z = ( sum_j(gamma_j * H_j'* H_j) )^(-1) * ( sum_j(gamma_j * H_j'*
            # xi_j) )
...
def KernelConstraintProj(u, size_k_full, psf_radius):
    ...
    u_norm = np.tile(
        u_sum, [u_proj.shape[1], u_proj.shape[2], 1]).transpose(2, 0, 1)
...
def precompute_H_hat_D(z_hat, size_z, rho):
    ...
    zhat_mat = np.transpose(z_hat.transpose(
        0, 1, 3, 2).reshape(n, k, -1), [2, 0, 1])
...
def solve_conv_term_Z(dhatT, dhatTdhat, xi_hat, gammas, size_z):
    ...
    # Invert
    scInverse = np.divide(np.ones((1, sx * sy)), rho *
                          np.ones((1, sx * sy)) + dhatTdhat.T)

    dhatT_dot_b = np.multiply(np.ma.conjugate(dhatT), b)
    dhatTb_rep = np.expand_dims(np.sum(dhatT_dot_b, axis=1), axis=1)
    x = 1.0 / rho * \
        (b - np.multiply(np.multiply(scInverse, dhatT), dhatTb_rep))
